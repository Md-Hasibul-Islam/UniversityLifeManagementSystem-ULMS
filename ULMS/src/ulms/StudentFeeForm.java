...
public class StudentFeeForm extends JFrame implements ActionListener{
    ...
    public StudentFeeForm() {
        super("ULMS - Student Fee Form");
	setBounds(500, 150, 395, 450);
	...
	JLabel l1 = new JLabel("Select Student ID");
	...
	contentPane.add(t3);
        ...
        try{
            Conn c = new Conn();
            ResultSet rs = c.s.executeQuery("select * from student where id = '"+c1.getSelectedItem()+"'");
            while(rs.next()){
	...
	comboBox.setModel(new DefaultComboBoxModel(new String[] {"Architecture","Civil","CSE","EEE","Mechanical","IPE","Textile","Accounting","Management","Marketing","English","Bangla"}));
	...
	comboBox_3.setModel(new DefaultComboBoxModel(new String[] {"BSc", "BBA", "BCom", "BA", "MSc", "MBA", "MA"}));
    ...
    public void actionPerformed(ActionEvent ae){
        try{
            ...
            if(ae.getSource() == b1){
                try{
                Conn con = new Conn();
                String sql = "insert into fee(id, name, fathers_name, course, dept, semester, fee_paid) values(?, ?, ?, ?, ?, ?, ?)";
